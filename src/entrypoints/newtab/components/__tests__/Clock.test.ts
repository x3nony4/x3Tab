...
describe('Clock', () => {
  it('renders time in HH:MM:SS format', () => {
    ...
    expect(wrapper.find('[class*="time"]').text()).toBe('14:05:30')
  })

  it('renders date in YYYY年MM月DD日 星期X format', () => {
    setTime(new Date(2026, 5, 23))
    const wrapper = mount(Clock)
    expect(wrapper.find('[class*="date"]').text()).toBe('2026年06月23日 星期二')
  })

  it('updates time every second', async () => {
    setTime(new Date(2026, 5, 23, 23, 59, 59))
    const wrapper = mount(Clock)
    expect(wrapper.find('[class*="time"]').text()).toBe('23:59:59')

    vi.advanceTimersByTime(1000)
    await wrapper.vm.$nextTick()

    expect(wrapper.find('[class*="time"]').text()).toBe('00:00:00')

    vi.advanceTimersByTime(1000)
    await wrapper.vm.$nextTick()

    expect(wrapper.find('[class*="time"]').text()).toBe('00:00:01')
  })
  ...
  it('text elements are not selectable', () => {
    ...
    const time = wrapper.find('[class*="time"]')
    const date = wrapper.find('[class*="date"]')
